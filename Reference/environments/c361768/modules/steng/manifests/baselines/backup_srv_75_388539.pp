class steng::baselines::backup_srv_75_388539(
  ...
  steng::ctdafile{"/etc/resolv.conf":
    container => $my_container,
    categ => $my_categ,
	dc => $my_dc,
  }
  ...
  # Consider to remove the resource for resolv.conf or modify it 
  # Consider using the AD for NTP
  # Remove local user accounts (and see what to do with auth.conf)
 ...
 class{steng::backup::ntp_f:
  ...
  steng::ctdafile{"/usr/local/steng/backup/bin/WKEBackupsReport.rb":
    container => $my_container,
    categ => $my_categ,
    dc => $my_dc,
    owner => root,
