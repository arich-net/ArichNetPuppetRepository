class steng::baselines::backup_srv_76_372617(
  ...
  steng::ctdafile{"/root/.ssh/id_rsa.pub":
   ...
   steng::ctdafile{"/root/.ssh/known_hosts":
     container => $my_container,
     categ => $my_categ,
 	dc => $my_dc,
 	require => File['/root/.ssh'],
  ...
  # NEXUS EC PACKAGES
  include steng::backup::ecpackages
  ...
  file{"/etc/sudoers.d/90-ec":
    ensure => present,
    owner => "root",
    group => "root",
    mode => 0440, 
    source => "puppet:///modules/steng/backup/etc/sudoers.d/90-ec",
  }
